...
in {
  ...
  in lib.mkIf cfg.enable {
    ...
    os.services.slskd.settings.remote_configuration = lib.mkForce false;

    os.users.users = lib.optionalAttrs (cfg.user == defaultUser) {
      "${defaultUser}" = {
        group = cfg.group;
        isSystemUser = true;
      };
    ...
    os.users.groups = lib.optionalAttrs (cfg.group == defaultUser) {
      "${defaultUser}" = {};
    };

    os.systemd.services.slskd = {
    ...
    os.networking.firewall.allowedTCPPorts = lib.optional cfg.openFirewall cfg.settings.soulseek.listen_port;

    os.services.nginx = lib.mkIf (cfg.domain != null) {
