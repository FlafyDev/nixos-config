{ config, osConfig, lib, utils, pkgs, inputs, combinedManager, ... }@host:

with lib;

let

  configurationPrefix = optionalString (versionAtLeast osConfig.system.stateVersion "22.05") "cm-";
  ...
  startScript = cfg:
    ''
      ...
      mkdir -p -m 0700 "$root/var/lib/private" "$root/root" /run/cm-containers
      ...
      exec ${osConfig.systemd.package}/bin/systemd-nspawn \
        --keep-unit \
  ...
  kernelVersion = osConfig.boot.kernelPackages.kernel.version;
...

{
          ...
          options = {
            config = mkOption {
              ...
              type = lib.mkOptionType {
                ...
                merge = loc: defs: ((import combinedManager).nixosSystem {
                  inherit inputs;
                  configuration = {
                    modules = [
                      ({options, ...}: {
                    ...
                    ] ++ (utils.getModules (toString ../../modules)) ++ (map (x: x.value) defs);
  ...
  config = mkMerge [
    {
      os.warnings = optional (!osConfig.boot.enableContainers && config.containers != {})
        "containers.<name> is used, but boot.enableContainers is false. To use containers.<name>, set boot.enableContainers to true.";
    }

      # os = mapAttrs (key: value: mkIf (osConfig.boot.enableContainers) value) {
      #   services.jellyfin = {
      #     enable = true;
      #   };
      # };
    {
      os = mapAttrs (key: value: mkIf (osConfig.boot.enableContainers) value) (let
        unit = {
      ...
      in {
        warnings =
          (optional (osConfig.virtualisation.containers.enable && versionOlder osConfig.system.stateVersion "22.05") ''
        ...
        systemd.services = listToAttrs (filter (x: x.value != null) (
          ...
          [{ name = "cmcontainer@"; value = unit; }]
          # declarative containers
          ++ (mapAttrsToList (name: cfg: nameValuePair "cmcontainer@${name}" (let
              containerConfig = cfg // (
            ...
            in
              recursiveUpdate unit {
                ...
                environment.root = if containerConfig.ephemeral then "/run/cm-containers/%i" else "${stateDirectory}/%i";
              } // (
              optionalAttrs containerConfig.autoStart
                {
                  ...
                  restartTriggers = [
                    ...
                    osConfig.environment.etc."${configurationDirectoryName}/${name}.conf".source
                  ];
        ...
        # Generate a configuration file in /etc/cm-containers for each
        ...
        services.udev.extraRules = optionalString osConfig.networking.networkmanager.enable ''
