let
  combinedManager = import (builtins.fetchTarball {
    url =
      "https://github.com/flafydev/combined-manager/archive/18fb4f6fd42bb6cceb9fc095897c1deb43f20c37.tar.gz";
...
in combinedManager.mkFlake {
  ...
  configurations = builtins.mapAttrs (host: cfg:
    cfg // {
      modules = [
        ({ lib, ... }: {
          imports = (import ./utils { inherit lib; }).getModulesForHost host;
        })
      ];
    }) {
  ...
  outputs = inputs@{ self, flake-parts, ... }:
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { pkgs, ... }: {
        devShells.default =
          pkgs.mkShell { packages = [ pkgs.nixd pkgs.nil pkgs.nixfmt ]; };
      };
    };
}
